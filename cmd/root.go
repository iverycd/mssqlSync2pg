...
func startRun(connStr *connect.DbConnStr) {
	...
	if selFromYml != true {
		...
		fkRet := []string{"FKCreate skip"}
		// 创建视图
		viewRet := db.ViewCreate(logDir)
		// 创建触发器
		//triRet := db.TriggerCreate(logDir)
		triRet := []string{"TriggerCreate skip"}
		// 以上对象迁移结果追加到切片,进行整合
...
//		sqlStr = "SELECT t.name AS table_name FROM sys.tables t INNER JOIN sys.schemas s ON t.schema_id = s.schema_id WHERE t.type = 'U' "
...
//				tabName = strings.ReplaceAll(tabName, "*", "%")
//				buffer.WriteString(" and t.name not like " + "'" + tabName + "'" + " ")
//			} else {
//				buffer.WriteString(" and t.name not like " + "'" + tabName + "'" + " ")
//			}
//		}
...
//	} else {
//		sqlStr = "SELECT t.name AS table_name FROM sys.tables t INNER JOIN sys.schemas s ON t.schema_id = s.schema_id WHERE t.type = 'U';" // 获取库里全表名称
//	}
//	fmt.Println(sqlStr)
//	// 查询下源库总共的表，获取到表名
...
func fetchTableMap(pageSize int, excludeTable []string) (tableMap map[string][]string) {
	...
	if excludeTable != nil {
		//sqlStr = "select table_name from information_schema.tables where table_schema=database() and table_type='BASE TABLE' and table_name not in ("
		sqlStr = "SELECT lower(t.name) AS table_name FROM sys.tables t INNER JOIN sys.schemas s ON t.schema_id = s.schema_id WHERE t.type = 'U' "
		buffer := bytes.NewBufferString("")
		for _, tabName := range excludeTable {
			//if index < len(excludeTable)-1 {
			//	buffer.WriteString("'" + tabName + "'" + ",")
		...
		sqlStr = "SELECT lower(t.name) AS table_name FROM sys.tables t INNER JOIN sys.schemas s ON t.schema_id = s.schema_id WHERE t.type = 'U';" // 获取库里全表名称
	}
	fmt.Println(sqlStr)
	// 查询下源库总共的表，获取到表名
	rows, err := srcDb.Query(sqlStr)
...
//
//			if !tableOnly {
//				sqlFullList, err = prepareSqlStr(tableName, pageSize)
//				if err != nil {
//					log.Error("prepareSqlStr for table %s failed: %v", tableName, err)
//					return
//				}
//			} else {
//				sqlFullList = append(sqlFullList, "")
...
func prepareSqlStr(tableName string, pageSize int) (sqlList []string) {
	...
	sql1 := fmt.Sprintf("SELECT min(lower(column_name)) column_name FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE  TABLE_CATALOG = DB_NAME() AND TABLE_NAME = '%s' group by table_name,column_name,ORDINAL_POSITION ORDER BY ORDINAL_POSITION;", tableName)
	rows, err := srcDb.Query(sql1)
	defer rows.Close()
	if err != nil {
	...
	sql2 := "/* mssqlSync2pg */" + "select CEILING(count(*)/" + strconv.Itoa(pageSize) + ") as total_page_num from " + "" + tableName + ""
	//以下是直接使用QueryRow
	...
	for i := 0; i <= totalPageNum; i++ { // 使用小于等于，包含没有行数据的表
		...
		sqlStr = "SELECT t.* FROM (SELECT " + buffer1.String() + " FROM " + "" + tableName + "" + " ORDER BY " + buffer1.String() + " OFFSET " + strconv.Itoa(i*pageSize) + " ROWS FETCH NEXT " + strconv.Itoa(pageSize) + " ROWS ONLY) temp LEFT JOIN " + "" + tableName + "" + " t ON " + buffer2.String() + ";"
		sqlList = append(sqlList, strings.ToLower(sqlStr))
	}
	return sqlList
}

//func prepareSqlStr(tableName string, pageSize int) (sqlList []string, err error) {
//	var scanColPk string   // 每个表主键列字段名称
//	var colFullPk []string // 每个表所有主键列字段生成的切片
//	var totalPageNum int   // 每个表的分页查询记录总数，即总共有多少页记录
//	var sqlStr string      // 分页查询或者全表扫描sql
//	//先获取下主键字段名称,可能是1个，或者2个以上组成的联合主键
//	//sql1 := "SELECT lower(COLUMN_NAME) FROM information_schema.key_column_usage t WHERE constraint_name='PRIMARY' AND table_schema=DATABASE() AND table_name=? order by ORDINAL_POSITION;"
//	sql1 := fmt.Sprintf("SELECT LOWER(COLUMN_NAME) AS column_name FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE  TABLE_CATALOG = DB_NAME() AND TABLE_NAME = '%s' ORDER BY ORDINAL_POSITION;", tableName)
//	rows, err := srcDb.Query(sql1)
//	defer rows.Close()
//	if err != nil {
//		log.Fatal(sql1, " exec failed ", err)
...
//		colFullPk = append(colFullPk, scanColPk)
//	}
//	fmt.Println("colFullPk:", colFullPk)
//	// 没有主键，就返回全表扫描的sql语句,即使这个表没有数据，迁移也不影响，测试通过
//	if colFullPk == nil {
//		sqlList = append(sqlList, "select * from "+""+tableName+"")
//		return sqlList, nil
...
//	//以下是直接使用QueryRow
//	err = srcDb.QueryRow(sql2).Scan(&totalPageNum)
//	fmt.Println("totalPageNum:", totalPageNum)
//	if err != nil {
//		log.Fatal(sql2, " exec failed ", err)
//		return sqlList, err
//	}
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.mssqlSync2pg.yaml)")
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name ".mssqlSync2pg" (without extension).
		viper.AddConfigPath(home)
		viper.SetConfigName(".mssqlSync2pg")
	}
